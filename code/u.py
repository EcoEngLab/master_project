...
λ_u = np.random.uniform(0.1, 1, N)  # 每个 consumer 的 uptake scaling factor

N_modules = 5  # module number of consumer to resource
s_ratio = 10.0  

# Generate uptake matrix
u = param_umean.modular_uptake(N, M, N_modules, s_ratio, λ_u)  # uptake matrix
...
l = param_umean.generate_l_tensor(N, M, N_modules, s_ratio, λ)  # a tensor for all consumers' leakage matrices
...
popt, pcov = curve_fit(log_func, u_mean, final_CUE)
a, b = popt
x_fit = np.linspace(min(u_mean), max(u_mean), 100)
y_fit = log_func(x_fit, *popt)

# u_mean vs. CUE
plt.scatter(u_mean, final_CUE, label='Data', color='blue')
plt.plot(x_fit, y_fit, color='red',
         label=f'Fitted Curve: a={a:.2f}, b={b:.2f}, R²={r2:.2f}')
plt.xlabel("Uptake mean")
plt.ylabel("Final CUE")
plt.title("Logarithmic Fit: Uptake mean vs. Final CUE")
plt.legend()
plt.show()
